defmodule NbaLinesServer.Authentication do
    @moduledoc """
    Provides methods to login a user by username and password both for the web
    ...
    def login_by_username_and_pass(conn, username, given_pass, opts) do
      ...
      user = repo.get_by(NbaLinesServer.User, username: username)
    ...
    @doc "Login by API username and password to generate and return a token"
    def api_login_by_username_and_pass(conn, username, password, opts) do
      repo = Keyword.fetch!(opts, :repo)
      user = repo.get_by(NbaLinesServer.User, username: username)
      cond do
